...
프로젝트는 프론트엔드와 백엔드를 분리하여 개발하며, 문서와 데이터는 별도의 디렉터리에서 관리합니다.
...
│   └── [GitWorkflow.md](http://GitWorkflow.md)

│

├── database/          # 개발용 데이터베이스 및 SQL
...
| database | 데이터베이스 및 SQL 관리 |

---

# Git Workflow

프로젝트는 Git을 이용하여 기능 단위로 버전을 관리합니다.

문서 작성 단계에서는 `main` 브랜치를 사용하며, 기능 개발부터는 `feature` 브랜치를 생성하여 작업합니다.

## 브랜치 전략

| 브랜치 | 용도 |
|---------|------|
| `main` | 문서 관리 및 안정적인 코드 유지 |
| `feature/*` | 새로운 기능 개발 |
| `fix/*` | 버그 수정 |
...
커밋 메시지는 작업 목적을 쉽게 파악할 수 있도록 다음 규칙을 사용합니다.
...
기능 구현 전 GitHub Issue를 생성하여 작업 내용을 관리합니다.
...
Issue는 기능 단위로 생성하며, 하나의 Issue는 하나의 기능 구현을 원칙으로 합니다.
